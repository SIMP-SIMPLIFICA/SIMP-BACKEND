...
export async function authenticate(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    // Attach user and permissions to request (Cast as any to bypass build errors)
  ...
  return async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!hasRequiredRole) {
      logSecurity('insufficient_role_access', 'medium', {
        ...
        requiredRoles: roles,
        userRoles,
        endpoint: (request as any).routeOptions?.schema,
        ip: request.ip
      })
...
export function requirePermissions(permissions: string[]) {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!req.permissions) {
      req.permissions = await db.getUserPermissions(req.user.id)
    }
    ...
    if (!hasAllPermissions) {
      ...
      logSecurity('insufficient_permissions_access', 'medium', {
        ...
        endpoint: req.routeOptions?.schema,
        ip: request.ip
      })
...
export function requireSelfOrAdmin(userIdParam: string = 'id') {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const isAdmin = await db.hasRole(user.id, 'admin')
...
export function requireUserRateLimit(maxRequests: number = 100, windowMs: number = 60000) {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const key = `user_rate_limit:${user.id}:${(request as any).routeOptions?.schema}`
...
export function require2FA() {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (user.twoFactorEnabled) {
      const sessionKey = `2fa_verified:${user.id}`
    ...
    switch (resourceType) {
      ...
      default:
        try {
          ...
          isOwner = resource?.userId === user.id
        } catch (error) {
...
// Hook factories
export function createAuthHook(middleware: any[]) {
  return {
...
export function getUserContext(request: FastifyRequest) {
  ...
  return {
    ...
    isModerator:
      user?.roles?.some((ur: any) => ['admin', 'moderator'].includes(ur.role.name)) || false
  }
}